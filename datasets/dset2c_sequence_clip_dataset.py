...
class Dset2CSequenceClipDataset(Dataset):
    ...
    Clips are recovered only from canonical manifest rows. This dataset never
    scans raw sequence directories and never introduces random pair splits.
    """
    ...
    def _build_clips(self, rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for seq_id, seq_rows in sorted(by_seq.items()):
            adjacent_rows = [
                row for row in seq_rows if str(row["pair_type"]) == "adjacent" and int(row["k"]) == 1
            ]
            adjacent_rows.sort(key=lambda row: (int(row["frame_a"]), int(row["frame_b"]), float(row["timestamp_a"])))
            if len(adjacent_rows) < self.clip_len - 1:
                self._bump_skip("clip:not_enough_adjacent_rows")
                continue

            ts_diffs = [
                float(row["timestamp_b"]) - float(row["timestamp_a"])
                for row in adjacent_rows
                if float(row["timestamp_b"]) > float(row["timestamp_a"])
            ]
            median_dt = float(np.median(np.asarray(ts_diffs, dtype=np.float64))) if ts_diffs else 0.0
            for start in range(0, len(adjacent_rows) - (self.clip_len - 1) + 1):
                window = adjacent_rows[start:start + self.clip_len - 1]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        return {
            "images": images,
            "frame_ids": torch.tensor([int(frame.frame_index) for frame in frames], dtype=torch.int64),
            ...
            "provenance": {
                "manifest_path": str(self.manifest_path),
                "sequence_id": str(sample["sequence_id"]),
                "frame_ids": [int(frame.frame_index) for frame in frames],
            },
        }
