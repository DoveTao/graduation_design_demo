...
TOOL_PATH = REPO_ROOT / "tools" / "run_thesis_three_axis_ablation.py"
SCRIPT_PATH = REPO_ROOT / "scripts" / "run_thesis_three_axis_ablation.sh"
MANIFEST_PATH = REPO_ROOT / "checkpoints" / "final_clean_candidate_manifest.json"


class ThesisThreeAxisAblationStaticTest(unittest.TestCase):
    ...
    def test_script_contains_expected_commands(self) -> None:
        ...
        self.assertIn("tools/run_thesis_three_axis_ablation.py --verify-s5", text)

    def test_output_paths_are_correct(self) -> None:
        text = TOOL_PATH.read_text(encoding="utf-8")
        self.assertIn("thesis_three_axis_ablation.md", text)
        self.assertIn("AB2_thesis_three_axis_ablation_results.json", text)
