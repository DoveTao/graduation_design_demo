...
FILES = [
    REPO_ROOT / "checkpoints" / "MF1b_train_cv_config.json",
    REPO_ROOT / "tools" / "mf1_train_cv_chain_refiner.py",
    REPO_ROOT / "tools" / "mf1_eval_train_cv_chain_refiner.py",
    REPO_ROOT / "scripts" / "run_mf1b_train_cv.sh",
]


class TestMF1BStatic(unittest.TestCase):
    def test_expected_files_exist(self):
        for path in FILES:
            self.assertTrue(path.exists(), str(path))

    def test_runner(self):
        text = (REPO_ROOT / "scripts" / "run_mf1b_train_cv.sh").read_text(encoding="utf-8")
        self.assertIn("set -euo pipefail", text)
        self.assertIn("verify_final_candidate.sh", text)
        self.assertIn("checkpoints/MF1b_train_cv_results.json", text)
        self.assertIn("reports/MF1b_train_cv_report.md", text)

    def test_config_locked_s5_metrics(self):
        cfg = json.loads((REPO_ROOT / "checkpoints" / "MF1b_train_cv_config.json").read_text(encoding="utf-8"))
