...
run_1200_tmag_nodetach() {
  test -f "$C31_CKPT" || { echo "Missing C31_CKPT=$C31_CKPT"; exit 1; }
  python train_mvp.py \
    --set exp_name=O37_c31_smallk_anchor2_tmag_nodetach_1200 \
    "${common_args[@]}" \
    --set w_tdir_anchor=2.0 \
...
run_1200_tmag_nodetach_w02() {
  test -f "$C31_CKPT" || { echo "Missing C31_CKPT=$C31_CKPT"; exit 1; }
  python train_mvp.py \
    --set exp_name=O38_c31_smallk_anchor2_tmag_nodetach_w02_1200 \
    "${common_args[@]}" \
    --set w_tdir_anchor=2.0 \
    --set tmag_detach_features=False \
    --set use_tmag_global_bias=True \
...
run_1200_tdir_anchor3_nodetach() {
  test -f "$C31_CKPT" || { echo "Missing C31_CKPT=$C31_CKPT"; exit 1; }
  python train_mvp.py \
    --set exp_name=O39_c31_smallk_anchor3_tmag_nodetach_1200 \
    "${common_args[@]}" \
...
case "${1:-help}" in
  ...
  1200_tmag_nodetach) run_1200_tmag_nodetach ;;
  1200_tmag_nodetach_w02) run_1200_tmag_nodetach_w02 ;;
  1200_tdir_anchor3_nodetach) run_1200_tdir_anchor3_nodetach ;;
  *)
    echo "Usage: $0 {800|800_safe|1200|1200_safe|1200_odomselect|1200_smallkselect|1200_ignore_tiny_dt|1200_soft_tiny_dt|1200_dt_ramp|1200_smoothselect|1200_tmag_bias|1200_tmag_nodetach|1200_tmag_nodetach_w02|1200_tdir_anchor3_nodetach}"
    ...
    echo "O37/1200_tmag_nodetach keeps O36 but allows t_mag gradients into shared features."
    echo "O38/1200_tmag_nodetach_w02 keeps O37 and raises w_tmag to 0.2 for a drift-first scale probe."
    echo "O39/1200_tdir_anchor3_nodetach keeps O37 scale settings and raises w_tdir_anchor to 3.0 for direction-balanced small-k."
