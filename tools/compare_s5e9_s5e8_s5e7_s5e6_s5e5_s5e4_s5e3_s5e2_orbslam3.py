...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    table = [
        ...
        _row("S5E2 traceable dense", "454/454", s5e2, "minimal ridge"),
        _row("S5E3 traceable dense", "454/454", s5e3, "scale calibration"),
        _row("S5E4 traceable dense", "454/454", s5e4, "signed direction prior"),
        _row("S5E5 traceable dense", "454/454", s5e5, "temporal visual smoke"),
        _row("S5E6 traceable dense", "454/454", s5e6, "robust scale guard"),
        _row("S5E7 traceable dense", "454/454", s5e7, "guard dependent factorized geometry"),
        _row("S5E8 diagnostic ablation", "454/454", s5e8, "translation diagnostic ablation"),
        _row("S5E9 traceable dense", "454/454", s5e9, "scale-unit / small-motion / signed-direction fix"),
    ...
    payload = {
        ...
        "comparison_summary": s5e9.get("comparison_summary"),
        "comparison_to_orbslam3": s5e9.get("comparison_to_orbslam3"),
        "final_classification": s5e9.get("final_classification"),
        "summary": "S5E9 重点确认 raw scale explosion 是否来自单位/字段/参数化问题，并检查 small-motion 与 pair-order signed direction。",
    }
    ...
    lines += ["", "## comparison summary", str(payload["comparison_summary"]), "", "## comparison vs ORB-SLAM3", str(payload["comparison_to_orbslam3"]), "", "## caveats", "- S5E9 是实验性结果，不替换 S5 locked。", "- ORB-SLAM3 的 edge-level gaps 仍 unavailable。"]
