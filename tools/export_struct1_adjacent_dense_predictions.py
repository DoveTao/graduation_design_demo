...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    model, blob = load_trained_struct1(Path(args.candidate) / "struct1_model.pt")
    ...
    for idx in range(len(frames) - 1):
        ...
        conf = float(out["fine_geom"]["confidence"][0].mean().cpu().item())
        entropy = float(out["fine_geom"]["entropy"][0].mean().cpu().item())
        cycle_error = float(out["fine_geom"]["cycle_error"][0].mean().cpu().item())
        epipolar = float(out["fine_geom"]["epipolar_residual"][0].mean().cpu().item())
        rows.append(mm)
        conf_vals.append(conf)
        entropy_vals.append(entropy)
        cycle_vals.append(cycle_error)
        epi_vals.append(epipolar)
    ...
    payload = {
        ...
        "component_metrics": _summary(rows) | {
            "geometry_token_confidence_mean": float(np.mean(conf_vals)) if conf_vals else None,
            "softcorr_entropy_mean": float(np.mean(entropy_vals)) if entropy_vals else None,
            "cycle_consistency_mean": float(np.mean(cycle_vals)) if cycle_vals else None,
            "epipolar_residual_mean": float(np.mean(epi_vals)) if epi_vals else None,
        },
        "metrics_rows": rows,
    }
